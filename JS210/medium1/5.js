...
At the time `counter` is referenced in the template literal, it points to the function `counter` and `rate` has a value of `undefined`. This is due to variable hoisting: the function name and body has been hoisted and is accessible, but only the variable declaration `rate` has been hoisted; it does not yet have a value. Attempting to multiply the function by undefined returns NaN.
*/
...
/* 
This code will raise a SyntaxError. When declaring variables with `let`, we cannot have a variable and a function with the same name, because function declarations result in the creation of a variable. 
*/
